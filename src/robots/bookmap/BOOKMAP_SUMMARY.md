...
## חבילת Order Flow — זוגיות עם Bookmap (2026-06-26)
פיצ'רים שנוספו (כל אחד commit נפרד), מבוססי נתוני Binance WS חיים:
1. **בורר מצבי תצוגה** — שורת טאבים גלויה ורספונסיבית (נרות / בועות / מפת חום / מפת ספר),
   נגישה גם בטלפון; ברירת מחדל "מפת חום". כל מצב = preset שכבות; תפריט אינדיקטורים לכוונון עדין.
2. **CVD** — Cumulative Volume Delta בפאנל מתחת לגרף (קונים אגרסיביים − מוכרים), ירוק/אדום
   מעל/מתחת ל-baseline, מסונכרן בציר הזמן עם הגרף הראשי.
3. **Iceberg** — זיהוי לפי מודל ספיגה: נפח חוזר שנאכל ברמה בלי שהגודל הגלוי ב-DOM קטֵן → הדגשה.
4. **Order Book Imbalance** — מד יחס נזילות קנייה/מכירה בראש ה-DOM (% + סנטימנט).
5. **Large Lot Tracker** — לוכד עסקאות ענק, halo זוהר לבועות הגדולות + חץ על ציר המחיר עם גודל $.
6. **Strength Level** — רמות שבהן נפח שבוצע ≫ נזילות יושבת (נזילות מוסתרת) מסומנות ב-★.
7. **VWAP** — קו ממוצע משוקלל-נפח לסשן (זהב, מקווקו). קווי BBO כבר קיימים (BBORibbon).
8. **POC** — הדגשת רמת המחיר עם הנפח הגבוה ביותר בתוך ה-Volume Profile.
9. **Spoofing/Layering** — זיהוי קירות גדולים שמופיעים ונמחקים לפני שהמחיר מגיע → סימון 👻 על ה-heatmap.
10. **Record & Replay** — קיים; כל המנועים החדשים משוחזרים גם ב-replay; מחוון "שחזור %" בסרגל.
11. **Multibook** — נדחה כ-TODO (ראה `MULTIBOOK_TODO.md`) כדי לא לסכן יציבות.

מנועים חדשים: `CVDEngine`, `LargeLotEngine`, `VWAPEngine`, `SpoofEngine` (+`IcebergStopsEngine`
שודרג). כולם ניזונים מאותו `handleTrade`/`onBook` הקיים — לא נשבר ה-state או חיבור ה-WebSocket.
הביצועים: כל הציור על Canvas/rAF, פאנלים נפרדים עם rAF משלהם, מנועים עם ring-buffers חסומים.
